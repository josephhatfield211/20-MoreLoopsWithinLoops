...
def run_test_largest_negative_number():
    ...
    print('Expected and actual are', expected, 'and', actual)
...
def first_is_elsewhere_too(seq_seq):
    ...
    for j in range(len(seq_seq)-1):
        sequenceinit = seq_seq[0]
        if len(sequenceinit) > 0:
            for k in range(len(sequenceinit)):
                thing = sequenceinit[k]
                for i in range(len(seq_seq[j+1])):
                    sequence = seq_seq[j+1]
                    if sequence[i] == thing:
                        real = True
    return real
